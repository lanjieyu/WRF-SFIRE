...
function addViewOptions(view) {
    var folder = gui.addFolder("视角");
    if (view) {
        folder.add(view, "reset").name("复位");
        folder.add(view, "viewUp").name("顶视图");
        folder.add(view, "viewDown").name("底视图");
        folder.add(view, "viewEast").name("东视图");
        folder.add(view, "viewWest").name("西视图");
        folder.add(view, "viewNorth").name("北视图");
        folder.add(view, "viewSouth").name("南视图");
        folder.add(view, "axisVisible").name("坐标轴");
    }
    return folder;
}
function addCloudOptions(cloud) {
    var folder = gui.addFolder("PM2.5之点状体数据分析");
    if (cloud) {
        var count = 21;
        var vm = {
            setColor: function(c1, c2) {
                cloud.volume.color1 = c1;
                cloud.volume.color2 = c2;
            },
            aqiModel: function() { this.setColor(0x00ff00, 0xff0000); },
            fireModel: function() { this.setColor(0xff0000, 0x00ff00); },
            streamModel: function() { this.setColor(0x0000ff, 0xa00000); },
            cloudModel: function() { this.setColor(0xffffff, 0x000000); },
            smokeModel: function() { this.setColor(0x000000, 0x333333); },
            ...
            get index() { return this.dataIndex; },
            set index(value) {
                this.dataIndex = ((value % count) + count) % count; // 循环索引
                var fn = "./data/pm25/" + String(this.dataIndex).padStart(2, "0") + ".json";

                // 缓存机制：首次加载缓存 Promise
                if (!this.frames[fn]) {
                    this.frames[fn] = cloud.load(fn);
                } else {
                    // 已缓存，仍然触发刷新显示
                    cloud.load(fn);
                }
        ...
        function animate(time) {
            ...
            if (!vm.anim || !cloud.volume.visible) return;
            if (time - lastTime >= vm.speed) {
                vm.index++;
                lastTime = time;
            }
        }
        ...
        // GUI 控件
        folder.add(cloud.volume, "visible").name("显示");
        folder.add(cloud.volume, "threshold", 0, 5, 1).name("阈值");
        folder.add(cloud.volume, "blendAdd").name("色彩增强");
        folder.addColor(cloud.volume, "color1").name("开始颜色").listen();
        folder.addColor(cloud.volume, "color2").name("结束颜色").listen();

        folder.add(vm, "aqiModel").name("AQI模式");
        folder.add(vm, "fireModel").name("火焰模式");
        folder.add(vm, "streamModel").name("虚幻模式");
        folder.add(vm, "cloudModel").name("白云模式");
        folder.add(vm, "smokeModel").name("烟雾模式");
        folder.add(vm, "next").name("单步动画");
        folder.add(vm, "anim").name("动画");
        folder.add(vm, "speed", 50, 1000, 10).name("播放速度(ms)");
        folder.add(vm, "index", 0, count - 1, 1).name("时间轴").listen();

        // 切片控制
        var clipfolder = folder.addFolder("切片");
        clipfolder.close();
        var clip = cloud.clip;
        if (clip) {
            clipfolder.add(clip, "enable").name("显示");
            clipfolder.add(clip, "x", -0.5, 0.5, 0.01).name("x切片").listen();
            clipfolder.add(clip, "y", -0.5, 0.5, 0.01).name("y切片").listen();
            clipfolder.add(clip, "z", -0.5, 0.5, 0.01).name("z切片").listen();
            clipfolder.add(clip, "opacity", 0, 1, 0.1).name("切片透明度");
            clipfolder.add(clip, "smooth").name("切面纹理平滑");
        }
    }
    return folder;
}
...
//         folder.add(vm, "next").name("单步动画");
//         folder.add(vm, "anim").name("动画");
//         folder.add(vm, "index", 0, 20, 1).name("时间轴").listen();
...
//             clipfolder.add(clip, "z", -0.5, 0.5, 0.01).name("z切片").listen();
//             clipfolder.add(clip, "opacity", 0, 1, 0.1).name("切片透明度");
//             // clipfolder.add(clip, "volumeVisible").name("显示体数据");
//             clipfolder.add(clip, "smooth").name("切面纹理平滑");
//         }
//     }
//     return folder;
...
function addPm252Options(pm252) {
    var folder = gui.addFolder("PM2.5之体数据分析");
    if (pm252) {
        var count = 73;
        var vm = {
            dataIndex: 0,
        ...
        folder.add(pm252, "visible").name("显示");
        folder.add(pm252, "threshold0", 1, 100, 1).name("阈值0");
        folder.add(pm252, "threshold1", 0, 100, 1).name("阈值1");
        folder.add(vm, "anim").name("动画");
        folder.add(vm, "index", 0, 72, 1).name("时间轴").listen();
    }
    return folder;
}

function addWindOptions(wind) {
    var folder = gui.addFolder("风");
    if (wind) {
        folder.add(wind, "visible").name("显示");
        folder.add(wind, "anim").name("动画");
        folder.add(wind, "play").name("单步动画");
        folder.add(wind, "reset").name("复位");
        folder.add(wind.windPlane, "visible").name("风速背景");
    }
    return folder;
}

function addHighOptions(high) {
    var folder = gui.addFolder("高度");
    if (high) {
        folder.add(high, "visible").name("显示");
        folder.add(high, "wireframe").name("网格");
        folder.add(high, "hscale", 1, 10, 1).name("高度拉伸");
    }
    return folder;
}
...
var PointVolumeMesh = function (_THREE$Points) {
    ...
    /**
     * @param {*} data 要分析的数据数组（三维数组的展开形式）
     * @param {*} width 数组宽度数（第一维度大小）
     * @param {*} height 数据高度数（第二维度大小）
     * @param {*} depth 数据的（第三维度大小）
...
// }
/*体数据Mesh类
  根据三维数据生成体数据的Mesh，适合在三维空间显示数据的分布情况。
  TODO: 数据调整为按json格式存储（为减少数据量加快速度，可考虑使用图像存储数据）
*/
